...
local function Hide(Window, bind, notif)
	...
	bind = bind[2]
	if notif then
		Sobing:Notification({Title = "Interface Hidden", Content = "The interface has been hidden, you may reopen the interface by Pressing the UI Bind In Settings ("..tostring(bind)..")", Icon = "visibility_off"})
	end
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == SobingUI.Name and Interface ~= SobingUI then
			Hide(Interface.SmartWindow)
			Interface.Enabled = false
			Interface.Name = "Sobing-Old"
		end
	end
...
SobingUI.Enabled = false
SobingUI.SmartWindow.Visible = false
...
function Sobing:Notification(data) -- action e.g open messages
	task.spawn(function()
		data = Kwargify({
			Title = "Missing Title",
...
function Sobing:CreateWindow(WindowSettings)

	WindowSettings = Kwargify({
		Name = "Sobing UI Example Window",
		Subtitle = "",
		LogoID = "6031097225",
		LoadingEnabled = true,
	...
	if WindowSettings.KeySystem then
		...
		Draggable(SobingUI.MobileSupport, SobingUI.MobileSupport)
		...
		local direc = WindowSettings.KeySettings.SaveInRoot and "Sobing/Configurations/" .. WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/" or "Sobing/Configurations/" ..  WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/"

		if isfile and isfile(direc .. WindowSettings.KeySettings.FileName .. ".Sobing") then
			for i, Key in ipairs(WindowSettings.KeySettings.Key) do
				if string.find(readfile(direc .. WindowSettings.KeySettings.FileName .. ".Sobing"), Key) then
					Passthrough = true
					break
				end
			end
			...
			KeySystem.Action.Submit.Interact.MouseButton1Click:Connect(function()
				...
				if KeyFound then 
					...
					if WindowSettings.KeySettings.SaveKey then
						if writefile then
							writefile(direc .. WindowSettings.KeySettings.FileName .. ".Sobing", FoundKey)
						end
						Sobing:Notification({Title = "Key System", Content = "The key for this script has been saved successfully.", Icon = "lock_open"})
					end
				else
	...
	function Window:CreateTab(TabSettings)
		...
		function Tab:CreateSection(name : string)
			...
			function Section:CreateSlider(SliderSettings, Flag)
					...
					local Loop; Loop = RunService.Stepped:Connect(function()
						if SLDragging then
							...
							if SliderSettings.CurrentValue ~= NewValue then
								...
								-- Sobing.Flags[SliderSettings.Flag] = SliderSettings
							end
						else
				...
				if Flag then
					Sobing.Options[Flag] = SliderV
				end

				SobingUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
					Slider.Main.color.Color = Sobing.ThemeGradient
					Slider.Main.UIStroke.color.Color = Sobing.ThemeGradient
				end)
			...
			function Section:CreateToggle(ToggleSettings, Flag)    
				...
				SobingUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
					Toggle.toggle.color.Color = Sobing.ThemeGradient
					Toggle.toggle.UIStroke.color.Color = Sobing.ThemeGradient
				end)

				if Flag then
					Sobing.Options[Flag] = ToggleV
				end
			...
			function Section:CreateBind(BindSettings, Flag)
				...
				if Flag then
					Sobing.Options[Flag] = BindV
				end

				-- Sobing.Flags[BindSettings.Flag] = BindSettings
			...
			function Section:CreateDropdown(DropdownSettings, Flag)
				...
				function DropdownV:Set(NewDropdownSettings)
					...
					-- Sobing.Flags[DropdownSettings.Flag] = DropdownSettings
			...
			function Section:CreateColorPicker(ColorPickerSettings, Flag) -- by Rayfield/Throit
				...
				if Flag then
					Sobing.Options[Flag] = ColorPickerV
				end
		...
		function Tab:BuildConfigSection()
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					local success, returned = Sobing:SaveConfig(inputPath)
					if not success then
						Sobing:Notification({
							Title = "Interface",
							Icon = "error",
					...
					configSelection:Set({ Options = Sobing:RefreshConfigList() })
				end
			})
			...
			configSelection = Tab:CreateDropdown({
				...
				Options = Sobing:RefreshConfigList(),
				CurrentOption = {},
				MultipleOptions = false,
				SpecialType = nil,
				Callback = function(Value)
			...
			Tab:CreateButton({
				...
				Callback = function()
					local success, returned = Sobing:LoadConfig(selectedConfig)
					if not success then
						Sobing:Notification({
							Title = "Interface",
							Icon = "error",
			...
			Tab:CreateButton({
				...
				Callback = function()
					local success, returned = Sobing:SaveConfig(selectedConfig)
					if not success then
						Sobing:Notification({
							Title = "Interface",
							Icon = "error",
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					writefile(Sobing.Folder .. "/settings/autoload.txt", name)
			...
			Tab:CreateButton({
				...
				Callback = function()
					local name = selectedConfig
					delfile(Sobing.Folder .. "/settings/autoload.txt")
			...
			if isfile(Sobing.Folder .. "/settings/autoload.txt") then
				local name = readfile(Sobing.Folder .. "/settings/autoload.txt")
				loadlabel:Set( { Text = "Current autoload config: " .. name })
			end     
		end
		...
		local ClassParser = {
			["Toggle"] = {
				...
				Load = function(Flag, data)
					if Sobing.Options[Flag] then
						Sobing.Options[Flag]:Set({ CurrentValue = data.state })
					end
				end
			},
			["Slider"] = {
				...
				Load = function(Flag, data)
					if Sobing.Options[Flag] and data.value then
						Sobing.Options[Flag]:Set({ CurrentValue = data.value })
					end
				end
			},
			["Input"] = {
				...
				Load = function(Flag, data)
					if Sobing.Options[Flag] and data.text and type(data.text) == "string" then
						Sobing.Options[Flag]:Set({ CurrentValue = data.text })
					end
				end
			},
			["Dropdown"] = {
				...
				Load = function(Flag, data)
					if Sobing.Options[Flag] and data.value then
						Sobing.Options[Flag]:Set({ CurrentOption = data.value })
					end
				end
			},
			-- buggy as hell stil
			["Colorpicker"] = {
				...
				Load = function(Flag, data)
					...
					if Sobing.Options[Flag] and data.color then
						Sobing.Options[Flag]:Set({Color = HexToColor3(data.color)})
					end
				end
			}
		}
		...
		function Tab:BuildThemeSection()
			...
			}, "SobingInterfaceSuitePrebuiltCPC1") -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps

			local c2cp = Tab:CreateColorPicker({
				Name = "Color 2",
				Color = Color3.fromRGB(123, 201, 201),
			}, "SobingInterfaceSuitePrebuiltCPC2")

			local c3cp = Tab:CreateColorPicker({
				Name = "Color 3",
				Color = Color3.fromRGB(224, 138, 184),
			}, "SobingInterfaceSuitePrebuiltCPC3") 
			...
			c1cp:Set({
				Callback = function(Value)
					if c2cp and c3cp then
						Sobing.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Value or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))}
						SobingUI.ThemeRemote.Value = not SobingUI.ThemeRemote.Value
					end
				end
			})
			...
			c2cp:Set({
				Callback = function(Value)
					if c1cp and c3cp then
						Sobing.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, Value or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))}
						SobingUI.ThemeRemote.Value = not SobingUI.ThemeRemote.Value
					end
				end
			})
			...
			c3cp:Set({
				Callback = function(Valuex)
					if c2cp and c1cp then
						Sobing.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, Valuex or Color3.fromRGB(255,255,255))}
						SobingUI.ThemeRemote.Value = not SobingUI.ThemeRemote.Value
					end
				end
			})
		...
		local function SetFolder()
			...
			if WindowSettings.ConfigSettings.RootFolder ~= nil and WindowSettings.ConfigSettings.RootFolder ~= "" then
				Sobing.Folder = WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder
			else
				Sobing.Folder = WindowSettings.ConfigSettings.ConfigFolder
			end
		...
		function Sobing:SaveConfig(Path)
			if isStudio then return "Config system unavailable." end
			...
			local fullPath = Sobing.Folder .. "/settings/" .. Path .. ".Sobing"

			local data = {
				objects = {}
			}

			for flag, option in next, Sobing.Options do
		...
		function Sobing:LoadConfig(Path)
			if isStudio then return "Config system unavailable." end
			...
			local file = Sobing.Folder .. "/settings/" .. Path .. ".Sobing"
		...
		function Sobing:LoadAutoloadConfig()
			if isfile(Sobing.Folder .. "/settings/autoload.txt") then
				...
				local success, err = Sobing:LoadConfig(name)
				if not success then
					return Sobing:Notification({
						Title = "Interface",
						Icon = "sparkle",
						ImageSource = "Material",
		...
		function Sobing:RefreshConfigList()
			if isStudio then return "Config system unavailable." end

			local list = listfiles(Sobing.Folder .. "/settings")

			local out = {}
			for i = 1, #list do
				local file = list[i]
				if file:sub(-5) == ".Sobing" then
					local pos = file:find(".Sobing", 1, true)
					local start = pos
	...
	SobingUI.MobileSupport.Interact.MouseButton1Click:Connect(function()
		Unhide(Main, Window.CurrentTab)
		dragBar.Visible = true
		Window.State = true
		SobingUI.MobileSupport.Visible = false
